...
mod tests {
    ...
    #[test]
    fn recoverable_wfp_statuses_cover_privilege_and_support_gaps() {
        assert!(WfpStatusError::new("test", ERROR_ACCESS_DENIED).should_try_auto_elevation());
        assert!(WfpStatusError::new("test", ERROR_NOT_SUPPORTED).should_try_auto_elevation());
        assert!(!WfpStatusError::new("test", 87).should_try_auto_elevation());
    }

    #[test]
    fn elevated_powershell_parameters_quote_paths() {
        let params = elevated_powershell_parameters(
            Path::new(r"C:\Program Files\procwarden\helper.ps1"),
            Path::new(r"C:\Program Files\demo\demo.exe"),
            "rule-prefix",
            Path::new(r"C:\Temp\stop.signal"),
            Path::new(r"C:\Temp\ready.signal"),
        );
        assert!(params.contains(r#"-File "C:\Program Files\procwarden\helper.ps1""#));
        assert!(params.contains(r#"-Executable "C:\Program Files\demo\demo.exe""#));
        assert!(params.contains(r#"-StopFile C:\Temp\stop.signal"#));
        assert!(params.contains(r#"-ReadyFile C:\Temp\ready.signal"#));
    }

    #[test]
    #[ignore = "requires interactive UAC consent"]
    fn manual_elevated_firewall_helper_starts_with_uac_consent() {
        let executable = Path::new(r"C:\Windows\System32\cmd.exe");
        let guard = ElevatedFirewallGuard::spawn(executable)
            .expect("elevated firewall helper should start after UAC consent");
        drop(guard);
    }

    #[test]
    #[ignore = "requires interactive UAC consent"]
    fn manual_elevated_firewall_rules_lifecycle_is_effective() {
        let executable = Path::new(r"C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe");
        let guard = ElevatedFirewallGuard::spawn(executable)
            .expect("elevated firewall helper should start after UAC consent");

        let helper_exit = guard
            .process
            .try_wait_exit_code(0)
            .expect("helper process status should be queryable");
        assert!(
            helper_exit.is_none(),
            "helper should still be running while guard is alive, got exit code: {helper_exit:?}"
        );

        wait_until_rule_count_at_least(
            &guard._out_rule_name,
        ...
        let out_rule_name = guard._out_rule_name.clone();
        let in_rule_name = guard._in_rule_name.clone();

        drop(guard);

        wait_until_rules_absent(
            &[out_rule_name.as_str(), in_rule_name.as_str()],
    ...
    #[test]
    #[ignore = "requires interactive UAC consent and outbound connectivity"]
    fn manual_elevated_firewall_blocks_network_egress() {
        if !powershell_can_open_tcp_to_cloudflare() {
            eprintln!(
                "skip network-block assertion because baseline outbound connectivity is unavailable"
            );
            return;
        }
        ...
        let reachable_while_blocked = powershell_can_open_tcp_to_cloudflare();
        drop(guard);

        assert!(
            !reachable_while_blocked,
            "expected outbound connectivity to be blocked while elevated firewall rules are active"
        );
    }

    fn wait_until_rules_absent(rule_names: &[&str], timeout: Duration, poll: Duration) {
        let started = Instant::now();
        loop {
            let all_absent = rule_names.iter().all(|name| !firewall_rule_exists(name));
            if all_absent {
                return;
            }
            assert!(
                started.elapsed() < timeout,
                "temporary firewall rules were not removed in time"
            );
            sleep(poll);
        }
    }

    fn wait_until_rule_count_at_least(rule_name: &str, timeout: Duration, poll: Duration) {
        let started = Instant::now();
        loop {
            ...
            assert!(
                started.elapsed() < timeout,
                "expected firewall rule {rule_name} to exist, but it was not visible"
            );
            sleep(poll);
        }
    }
    ...
    fn firewall_rule_exists(rule_name: &str) -> bool {
        let output = Command::new("netsh")
            ...
            .output()
            .expect("firewall query should execute");

        let stdout = String::from_utf8_lossy(&output.stdout);
        let stderr = String::from_utf8_lossy(&output.stderr);
        let combined = format!("{stdout}\n{stderr}").to_ascii_lowercase();
        combined.contains(&rule_name.to_ascii_lowercase())
    }

    fn powershell_can_open_tcp_to_cloudflare() -> bool {
        let script = "try { $client = New-Object System.Net.Sockets.TcpClient; $async = $client.BeginConnect('1.1.1.1', 443, $null, $null); if (-not $async.AsyncWaitHandle.WaitOne(4000)) { exit 1 }; $client.EndConnect($async); $client.Close(); exit 0 } catch { exit 1 }";
        let output = Command::new(r"C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe")
            .args(["-NoProfile", "-NonInteractive", "-Command", script])
            .output()
            .expect("network probe command should execute");
