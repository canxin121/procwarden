...
const DENY_ALL_ACCESS_MASK: u32 = FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE;
...
pub(super) unsafe fn apply_access_plan(
    ...
    for path in &plan.deny_access_paths {
        let added = add_deny_all_access_ace(path, sid)?;
        if added {
...
pub(super) unsafe fn add_deny_all_access_ace(
    path: &Path,
    sid: *mut c_void,
) -> Result<bool, SandboxError> {
    add_deny_access_ace(path, sid, DENY_ALL_ACCESS_MASK)
}
